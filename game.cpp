	...
	, acc(9)
{
	...
	//helicopter.setOrigin(0.5, 0);
	//helicopter.setScale(-1, 1);
	//helicopter.setOrigin(0, 0);
}
		...
		*/
		...
		if ((counter = (counter + 1) % 1000) == 0)
			if (height < MAX_HEIGHT)
				height++;
		//if ((period = (period + 1) % WALL_PERIOD) == 0)
		for (auto &i : top_bricks)
		{
			for (auto &j : i)
			{
		...
		//if (w == 4)
		//{
		//		helicopter.setTextureRect(sf::IntRect(0, 2 * 62, 132, 62));
		//	}
		//else
